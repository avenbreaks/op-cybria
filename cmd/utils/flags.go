...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --trollup, --goerli, --sepolia, --holesky instead)",
		Value:    ethconfig.Defaults.NetworkId,
		Category: flags.EthCategory,
	}
	MainnetFlag = &cli.BoolFlag{
		Name:     "mainnet",
		Usage:    "Cybria-OP mainnet",
		Category: flags.EthCategory,
	}
	...
	TrollupFlag = &cli.BoolFlag{
		Name:     "trollup",
		Usage:    "Testnet Cybria Rollup network: pre-configured rollup test network",
		Category: flags.EthCategory,
	}
	RollupFlag = &cli.BoolFlag{
		Name:     "rollup",
		Usage:    "Mainnet Cybria Rollup network: pre-configured rollup mainnet network",
		Category: flags.EthCategory,
	}
	// Dev mode
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		if ctx.Bool(TrollupFlag.Name) {
			return filepath.Join(path, "trollup")
		}
		return path
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(TrollupFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "trollup")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, GoerliFlag, SepoliaFlag, HoleskyFlag, TrollupFlag)
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(TrollupFlag.Name):
	    genesis = core.DefaultTrollupGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):
