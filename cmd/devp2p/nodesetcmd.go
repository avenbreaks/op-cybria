...
func ethFilter(args []string) (nodeFilter, error) {
	...
	case "trollup":
	    filter = forkid.NewStaticFilter(params.TrollupChainConfig, core.DefaultTrollupGenesisBlock().ToBlock())
	case "rollup":
	    filter = forkid.NewStaticFilter(params.RollupChainConfig, core.DefaultRollupGenesisBlock().ToBlock())
	default:
